    ...
    const fillResult = await page.evaluate(`
      ...
      (() => {
        ...
        if (el.type === 'checkbox' || el.type === 'radio') {
          ...
          return { ok: true, actual: el.checked, strategy: 'checkbox-toggle', message: 'Checked: ' + el.checked, _target: ${JSON.stringify(target)} };
        }
        if (el.tagName === 'SELECT') return { ...fillSelect(el, ${JSON.stringify(value)}), _target: ${JSON.stringify(target)} };
        return { ...fillText(el, ${JSON.stringify(value)}), _target: ${JSON.stringify(target)} };
      })()
    `);

    // Step 2: Wait for async framework reactions (Oracle/Knockout, React state)
    await page.waitForTimeout(500);

    // Step 3: Verify actual value after delay
    const verifyResult = await page.evaluate(`
      ${PAGE_SCRIPTS}
      (() => {
        const el = resolveElement(${JSON.stringify(target)});
        if (!el) return { actual: null };
        return { actual: el.value || (el.type === 'checkbox' ? el.checked : null) || '' };
      })()
    `);

    // Step 4: Report — ok only if value actually stuck
    const result = { ...fillResult };
    result.actual = verifyResult.actual;
    result.duration = Date.now() - t0;

    // For text fills, verify the value matches
    if (fillResult.strategy === 'paste' || fillResult.strategy === 'char-by-char') {
      if (verifyResult.actual !== value) {
        result.ok = false;
        result.error = 'value_rejected';
        result.message = `Set "${value}" but field contains "${verifyResult.actual}". Framework cleared it.`;
      }
    }
